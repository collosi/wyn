...
impl BorrowChecker {
    ...
    fn extract_lifetime_facts<W: Write>(&mut self, fact_writer: &mut NemoFactWriter<W>, program: &Program) -> Result<()> {
        ...
        match decl {
                ...
                *block_counter += 1;
                ...
                for param in &entry_decl.params {
                    ...
                    *location_counter += 1;
                    ...
                    let location = Location { block: block_id, index: 0 };
                    fact_writer.write_location_fact(location_id, &location).map_err(Self::io_error)?;
                    fact_writer.write_var_def_fact(location_id, &param.name).map_err(Self::io_error)?;
                    fact_writer.write_lifetime_start_fact(lifetime_id, location_id, &param.name).map_err(Self::io_error)?;
                }
                
                // Analyze function body
                self.extract_expression_facts(fact_writer, &entry_decl.body, block_id, location_counter)?;
            }
            Declaration::Let(let_decl) => {
                ...
                *location_counter += 1;
                ...
                let location = Location { block: block_id, index: 0 };
                fact_writer.write_location_fact(location_id, &location).map_err(Self::io_error)?;
                fact_writer.write_var_def_fact(location_id, &let_decl.name).map_err(Self::io_error)?;
                fact_writer.write_lifetime_start_fact(lifetime_id, location_id, &let_decl.name).map_err(Self::io_error)?;
                
                // Analyze value expression
                self.extract_expression_facts(fact_writer, &let_decl.value, block_id, location_counter)?;
            }
                ...
                *block_counter += 1;
                ...
                for param_name in &def_decl.params {
                    ...
                    *location_counter += 1;
                    ...
                    fact_writer.write_lifetime_start_fact(lifetime_id, location_id, param_name).map_err(Self::io_error)?;
                }
                
                // Analyze function body
                self.extract_expression_facts(fact_writer, &def_decl.body, block_id, location_counter)?;
            }
            Declaration::Val(_) => {
        ...
        *location_counter += 1;
        ...
        match expr {
            Expression::Identifier(name) => {
                ...
                fact_writer.write_borrow_fact(borrow_id, location_id, name, lifetime_id).map_err(Self::io_error)?;
            }
            ...
            Expression::Lambda(lambda) => {
                ...
                for param in &lambda.params {
                    ...
                    *location_counter += 1;
                    ...
                    let param_location = Location { block: lambda_block, index: 0 };
                    fact_writer.write_location_fact(param_location_id, &param_location).map_err(Self::io_error)?;
                    fact_writer.write_var_def_fact(param_location_id, &param.name).map_err(Self::io_error)?;
                    fact_writer.write_lifetime_start_fact(lifetime_id, param_location_id, &param.name).map_err(Self::io_error)?;
                }
    ...
    fn write_borrow_checking_rules<W: Write>(&self, mut writer: W) -> Result<()> {
        ...
        writeln!(writer, "live_var(Var, Loc) :- lifetime_start(LifetimeId, Loc, Var).").map_err(Self::io_error)?;
        writeln!(writer, "").map_err(Self::io_error)?;
            ...
            Ok(mut engine) => {
                ...
                match engine.execute() {
                    ...
                    Err(nemo_error) => {
                        ...
                        Err(CompilerError::SpirvError(format!("Nemo execution failed: {:?}", nemo_error)))
                    }
                }
            }
            Err(nemo_error) => {
                ...
                // Convert Nemo error to our error type
                Err(CompilerError::SpirvError(format!("Failed to load Nemo program: {:?}", nemo_error)))
            }
        }
    }
...
impl BorrowCheckResult {
    ...
    pub fn print_errors(&self) {
        for error in &self.errors {
            match error {
                BorrowError::UseAfterMove { variable, use_location, move_location } => {
                    println!("Error: Use after move of variable '{}' at location {} (moved at {})", 
                             variable, use_location, move_location);
                }
                BorrowError::MultipleMutableBorrow { variable, borrow1, borrow2 } => {
                    println!("Error: Multiple mutable borrows of variable '{}' (borrows {} and {})", 
