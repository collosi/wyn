    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Type::Array(elem_ty, dims) => {
                let dim_str = dims.iter().map(|d| format!("[{}]", d)).collect::<String>();
...
pub fn builtin_zip_type() -> TypeScheme<Type> {
    ...
    let arr_b = PT::Constructed(Type::Array(Box::new(Type::I32), vec![1]), vec![]);

    let _tuple_result = PT::Constructed(Type::Tuple(vec![Type::I32, Type::I32]), vec![]);
