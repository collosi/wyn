...
impl TypeChecker {
    ...
    fn load_builtins(&mut self) -> Result<()> {
        ...
        let program = parser.parse().map_err(|e| 
            CompilerError::ParseError(format!("Failed to parse builtins: {:?}", e)))?;
    ...
    fn infer_expression(&mut self, expr: &Expression) -> Result<Type> {
        match expr {
            ...
            Expression::FunctionCall(func_name, args) => {
                // Get function type
                let mut func_type = self.env.get(func_name)
                ...
                for arg in args {
                    ...
                    match func_type {
                        ...
                        Type::Var(_) => {
                            ...
                            func_type = Type::Array(Box::new(Type::Tuple(vec![Type::I32, Type::I32])), vec![1]);
                        }
                        _ => {
                            return Err(CompilerError::TypeError(format!(
            ...
            Expression::Tuple(elements) => {
                let elem_types: Result<Vec<Type>> = elements.iter()
